...
const masterAppConfig = [
    ...
    { name: "AI", icon: "fa-solid fa-robot", type: 'proxied_url', content: 'https://duckduckgo.com/?q=e&ia=chat&duckai=1', pinned: true, pinnedToTaskbar: false, isGame: true },
    ...
    { name: "Roblox", icon: "https://cdn.jsdelivr.net/gh/sealiee11/randomluminalshii@main/imageonlyclone/projects/roblox%20copy/splash.png", type: 'action', content: 'openr0bl0x', pinned: true, pinnedToTaskbar: false, isGame: true },
    { name: "App store(fortnite, brawl stars, clash etc)", icon: "play.png", type: 'action', content: 'openaptoide', pinned: true, pinnedToTaskbar: false, isGame: true },

];

function openr0bl0x() {
  window.open('40810x.html', '_blank');
}
function openaptoide() {
  window.open('aptoide.html', '_blank');
}
const originalAlert = window.alert;

window.alert = function(message) {
    luminalAlert(message);
};

function luminalAlert(message, title = 'luminalOS Message') {
    if (document.querySelector('.luminal-alert-overlay')) {
        return;
    }

    const overlay = document.createElement('div');
    overlay.className = 'luminal-alert-overlay';

    const alertBox = document.createElement('div');
    alertBox.className = 'luminal-alert-box';

    const messageContainer = document.createElement('p');
    messageContainer.textContent = message;

    alertBox.innerHTML = `
        <div class="luminal-alert-header">
            <i class="fa-solid fa-circle-info"></i>
            <span>${title}</span>
        </div>
        <div class="luminal-alert-content">
            ${messageContainer.outerHTML}
        </div>
        <div class="luminal-alert-footer">
            <button class="os-button" id="luminal-alert-ok-btn">ok</button>
        </div>
    `;
    ...
    const okButton = document.getElementById('luminal-alert-ok-btn');
    okButton.focus();
...
function luminalConfirm(message, title = 'Confirmation') {
    return new Promise((resolve) => {
        ...
        alertBox.innerHTML = `
            ...
            <div class="luminal-alert-footer" style="gap: 10px;">
                <button class="os-button" id="luminal-confirm-cancel-btn" style="background-color:#555;">no</button>
                <button class="os-button" id="luminal-confirm-ok-btn">yes</button>
            </div>
        `;
        ...
        const okBtn = document.getElementById('luminal-confirm-ok-btn');
        const cancelBtn = document.getElementById('luminal-confirm-cancel-btn');
...
function createWindow(options) {
    ...
    const cheatEngineControls = document.createElement('div');
    cheatEngineControls.className = 'cheat-engine-controls';
    cheatEngineControls.innerHTML = ``
    ...
    header.appendChild(cheatEngineControls);
    ...
    updateTaskbar(); 
    return windowId;
    trackPlausibleEvent("open_app", {
    name: appName,
    url: appURL || "internal",
  });

  if (typeof plausible !== "undefined") {
    plausible("pageview", { u: appURL || "/" + appName });
  }
  

}

function togglePauseWindow(windowId, button) {
    const win = openWindows[windowId];
    if (!win) return;

    win.isPaused = !win.isPaused;
    const pauseOverlay = win.element.querySelector('.pause-overlay') || document.createElement('div');
    
    if (win.isPaused) {
        pauseOverlay.className = 'pause-overlay';
        win.element.querySelector('.window-content').appendChild(pauseOverlay);
        button.innerHTML = '<i class="fa-solid fa-play"></i>';
        button.title = "Resume Window";
    } else {
        if (pauseOverlay.parentNode) {
            pauseOverlay.parentNode.removeChild(pauseOverlay);
        }
        button.innerHTML = '<i class="fa-solid fa-pause"></i>';
        button.title = "Pause Window";
    }
}

function setWindowSpeed(windowId, speed) {
    const win = openWindows[windowId];
    if (!win) return;
    
    const iframe = win.iframeRef;
    if (iframe && iframe.contentWindow) {
        alert("Speed control for proxied apps is not supported.");
    } else {
        win.element.style.animationPlayState = 'running';
        win.element.style.animationDuration = `${1 / speed}s`;
    }
}
...
function makeDraggable(element, handle) {
    ...
    const startDrag = (e) => {
        if (e.target.closest('.window-control, .ce-btn')) return;
        e.preventDefault();
...
function createLoadingOverlay() {
  const overlay = document.createElement('div');
  overlay.className = 'proxy-loading-overlay';
  overlay.innerHTML = `
    <style>
      ...
      .proxy-loading-overlay.ready .particle {
        animation: superExplode 1.2s cubic-bezier(0.17, 0.67, 0.83, 0.67) forwards !important;
      }

      .proxy-loading-overlay.ready .border {
        animation: borderExplosion 1.2s ease-out forwards;
      }

      .proxy-loading-overlay.ready .glow-effect {
        animation: megaPulse 1.2s ease-out forwards;
      }

      .proxy-loading-overlay.ready .spinner-ring {
        animation: spinnerBlast 1.2s ease-out forwards !important;
      }

      .proxy-loading-overlay.ready .main-title {
        animation: titleBlast 1.2s cubic-bezier(0.68, -0.55, 0.265, 1.55) forwards;
      }

      .proxy-loading-overlay.ready .progress-bar {
        animation: progressExplode 1s ease-out forwards;
      }
      ...
      .proxy-loading-overlay.ready .tunnel-container {
        animation: tunnelFadeIn 0.8s ease-out 1s forwards;
      }
      ...
      @keyframes tunnelMove {
        from {
          transform: translate(-50%, -50%) translateZ(-1000px) rotateZ(0deg);
        }
        to {
          transform: translate(-50%, -50%) translateZ(0px) rotateZ(360deg);
        }
      }
      ...
      @keyframes superExplode {
        ...
        100% {
          transform: translate(calc(var(--tx) * 2), calc(var(--ty) * 2)) scale(3) rotate(720deg);
          opacity: 0;
          filter: blur(4px);
        }
      }
      ...
      @keyframes borderExplosion {
        ...
        30% {
          ...
          box-shadow: 
            0 0 100px rgba(0, 212, 170, 1),
            0 0 200px rgba(0, 122, 204, 1),
            0 0 300px rgba(120, 119, 198, 0.8),
      ...
      .proxy-background {
        ...
        height: 100%;
        background: 
          radial-gradient(circle at 20% 80%, rgba(0, 122, 204, 0.3) 0%, transparent 50%),
      ...
      .main-title {
        ...
        font-weight: 300;
        margin: 0 0 1rem 0;
        background: linear-gradient(135deg, #00d4aa, #007acc, #7877c6);
        background-size: 300% 300%;
        -webkit-background-clip: text;
        background-clip: text;
        -webkit-text-fill-color: transparent;
        animation: gradientShift 3s ease-in-out infinite;
        text-shadow: 0 0 30px rgba(0, 122, 204, 0.3);
      ...
      .progress-fill {
        height: 100%;
        background: linear-gradient(90deg, #007acc, #00d4aa, #7877c6);
      ...
      .particle:nth-child(1) { --tx: -200px; --ty: -200px; }
      .particle:nth-child(2) { --tx: -150px; --ty: 250px; }
      .particle:nth-child(3) { --tx: 180px; --ty: -180px; }
      .particle:nth-child(4) { --tx: 220px; --ty: 200px; }
      .particle:nth-child(5) { --tx: -250px; --ty: 100px; }
      .particle:nth-child(6) { --tx: 200px; --ty: -150px; }
      .particle:nth-child(7) { --tx: -180px; --ty: -220px; }
      .particle:nth-child(8) { --tx: 240px; --ty: 180px; }
    ...
    <div class="floating-particles">
      ...
      <div class="particle" style="left: 80%; top: 10%; animation-delay: -1.5s;"></div>
    ...
    <div class="loading-container">
      ...
      <div class="border">
        <h1 class="main-title">luminal</h1>
        ...
        <div class="status-text">Loading proxy...</div>
      </div>
    </div>

    <div class="proxy-background"></div>
  `;

  document.body.appendChild(overlay);

  const tunnel = overlay.querySelector('#tunnel');
  const colors = ['#007acc', '#00d4aa', '#7877c6'];
  
  for (let i = 0; i < 150; i++) {
    const particle = document.createElement('div');
    particle.className = 'tunnel-particle';
...
function updateProgress(overlay, percent) {
  ...
  if (statusText) {
    if (percent < 30) {
      statusText.textContent = 'Initializing proxy...';
    } else if (percent < 60) {
      statusText.textContent = 'Registering service worker...';
    } else if (percent < 90) {
      statusText.textContent = 'Loading destination...';
    } else {
...
function triggerExplosion(overlay, callback) {
  const loadingContainer = overlay.querySelector('.loading-container');
  const background = overlay.querySelector('.proxy-background');
  
  overlay.classList.add('ready');
  
  if (loadingContainer) {
    loadingContainer.style.transition = 'transform 1.2s cubic-bezier(0.68, -0.55, 0.265, 1.55), opacity 1s ease-out';
    loadingContainer.style.transform = 'scale(3) rotate(10deg)';
    loadingContainer.style.opacity = '0';
    loadingContainer.style.filter = 'blur(10px)';
  }
  
  if (background) {
    background.style.transition = 'transform 1.2s ease-out, opacity 1s ease-out, filter 0.8s ease-out';
    background.style.transform = 'scale(1.5)';
    background.style.opacity = '0';
    background.style.filter = 'brightness(2) blur(20px)';
  }
...
async function openProxiedUrl(destination, uvConfig, registerSW, createWindow, windowOptions = {}) {
  const normalizedUrl = normalizeUrl(destination);
  
  windowOptions.html = `
    <div id="proxy-loader-container" style="width: 100%; height: 100%; position: relative;">
    </div>
  `;
  ...
  setTimeout(() => {
    ...
    const loaderContainer = windowElement ? windowElement.querySelector('#proxy-loader-container') : null;
    
    if (!loaderContainer) {
      console.error('Could not find loader container');
      return;
    }
    ...
    registerSW()
      ...
      .catch((err) => {
        ...
        const statusText = overlay.querySelector('.status-text');
        if (statusText) {
          statusText.textContent = 'Error loading proxy';
          statusText.style.color = '#ff4444';
        }
        console.error('Proxy loading error:', err);
    ...
    const progressInterval = setInterval(() => {
      ...
      if (currentProgress >= 95 && !canRedirect) {
        currentProgress = 95;
      }
      
      if (currentProgress >= 100) {
        clearInterval(progressInterval);
        updateProgress(overlay, 100);
        
        setTimeout(() => {
          triggerExplosion(overlay, () => {
            const proxiedUrl = uvConfig.prefix + uvConfig.encodeUrl(normalizedUrl);
            ...
            loaderContainer.appendChild(iframe);
          });
        }, 100);
      }
      
      updateProgress(overlay, currentProgress);
    }, 45);
  }, 50);
}
...
function launchApp(appName) {
  ...
  if (window.__uv$config && typeof window.__uv$config.encodeUrl === 'function') {
    const encodedUrl = __uv$config.encodeUrl(fullUrl);
    return `/lumi/nal/${encodedUrl}`;
  } else {
    console.warn("UV config not ready, falling back to embed.html");
    return `embed.html#${fullUrl}`;
  }
};
    if (app.type === 'action') { 
...
const playGameInOS = (gameUrl, gameTitle) => {
    ...
    html: `<div style="width:100%; height:100%; display:flex; flex-direction:column; background:#000;">
               ...
               <iframe id="iframe-${gameId}" src="${gameUrl}" style="width:100%; flex-grow:1; border:none; position:relative; z-index:1;" onload="document.getElementById('loader-${gameId}').style.display='none'"></iframe>
           </div>`
});

openWindows[windowId].iframeRef = document.getElementById(`iframe-${gameId}`);
...
function enhanceGameWindow(windowId, gameId, gameTitle) {
    ...
    const userAgreed = await luminalConfirm(`Do you want to save your progress in "${gameTitle}" before closing?`);
...
async function sendReport(gameTitle) {
    ...
    const webhookUrl = 'youaintgettingshitcuzitsremoved';
    const payload = {
...
function applyPerformanceMode(level) {
    document.body.classList.remove('perf-low', 'perf-medium', 'perf-high', 'perf-ultra');
    document.body.classList.add(`perf-${level}`);
    localStorage.setItem('luminal_performance_mode', level);
}
...
function toggleFpsCounter(show) {
    ...
    if (show) {
        ...
        let frameCount = 0;
        function updateFps() {
            if (!isFpsCounterVisible) return;
            const now = performance.now();
            frameCount++;
            if (now > lastFrameTime + 1000) {
                fpsCounter.textContent = `FPS: ${Math.round(frameCount * 1000 / (now - lastFrameTime))}`;
                lastFrameTime = now;
                frameCount = 0;
            }
    ...
    localStorage.setItem('luminal_fps_counter', show);
}
...
async function startScreenRecording() {
    try {
        ...
        mediaRecorder.onstop = () => {
            ...
            const a = document.createElement('a');
            a.href = url;
            a.download = `luminal-recording-${Date.now()}.webm`;
            a.click();
            URL.revokeObjectURL(url);
        };
...
function stopScreenRecording() {
    if (mediaRecorder && mediaRecorder.state !== 'inactive') {
        mediaRecorder.stop();
        alert("Screen recording stopped. Download will begin shortly.");
    }
}
...
function applyTaskbarSettings() {
    ...
    const settings = {
        position: localStorage.getItem('luminal_taskbar_position') || 'bottom',
        size: localStorage.getItem('luminal_taskbar_size') || '40',
        autohide: localStorage.getItem('luminal_taskbar_autohide') === 'true',
        transparency: localStorage.getItem('luminal_taskbar_transparency') || '0.5',
        iconSize: localStorage.getItem('luminal_taskbar_iconSize') || '28',
        blur: localStorage.getItem('luminal_taskbar_blur') || '10'
    };

    document.body.className = document.body.className.replace(/taskbar-pos-\w+/g, '');
    document.body.classList.add(`taskbar-pos-${settings.position}`);
    
    taskbar.classList.toggle('auto-hide', settings.autohide);

    document.documentElement.style.setProperty('--taskbar-height', `${settings.size}px`);
    document.documentElement.style.setProperty('--taskbar-bg-transparency', settings.transparency);
    document.documentElement.style.setProperty('--taskbar-icon-size', `${settings.iconSize}px`);
    document.documentElement.style.setProperty('--taskbar-blur', `${settings.blur}px`);
}

function applyCursorTheme(themeName) {
    document.body.className = document.body.className.replace(/cursor-theme-\w+/g, '');
    if (themeName !== 'default') {
        document.body.classList.add(`cursor-theme-${themeName}`);
    }
    localStorage.setItem('luminal_cursor_theme', themeName);
}

function checkBatterySaver() {
    if (localStorage.getItem('luminal_battery_saver') !== 'true') return;
    if ('getBattery' in navigator) {
        navigator.getBattery().then(battery => {
            function updateBatteryStatus() {
                if (!battery.charging && battery.level < 0.2) {
                    applyPerformanceMode('low');
                    console.log('Battery saver activated.');
                }
            }
...
function startMacroRecording() {
    ...
    alert("Macro recording started. Perform your actions.");

    ['mousedown', 'mouseup', 'mousemove', 'keydown', 'keyup'].forEach(eventType => {
        document.addEventListener(eventType, recordMacroEvent, { capture: true });
    });
}

function recordMacroEvent(e) {
    if (!isRecordingMacro) return;
    const eventData = {
        type: e.type,
        target: e.target.id,
        ...
        key: e.key,
        keyCode: e.keyCode,
        timestamp: performance.now() - macroStartTime
    };
    recordedMacro.push(eventData);
}

function stopMacroRecording() {
    if (!isRecordingMacro) return;
    isRecordingMacro = false;
    alert(`Macro recording stopped. ${recordedMacro.length} events recorded.`);
    
    ['mousedown', 'mouseup', 'mousemove', 'keydown', 'keyup'].forEach(eventType => {
        document.removeEventListener(eventType, recordMacroEvent, { capture: true });
    });
}

function playMacro() {
    if (isRecordingMacro || recordedMacro.length === 0) return;
    alert("Playing back macro...");

    recordedMacro.forEach(eventData => {
        setTimeout(() => {
            let event;
            ...
            const targetElement = document.elementFromPoint(eventData.x, eventData.y) || document.body;
        ...
        if (!searchOverlay) {
            ...
            searchOverlay.innerHTML = `
                <div id="global-search-box">
                    <input type="text" id="global-search-input" placeholder="Search apps, files, and settings...">
...
function handleGlobalSearch(e) {
    ...
    const appResults = apps.filter(app => app.name.toLowerCase().includes(query));
...
function openSettings(options) {
    ...
    const perfMode = localStorage.getItem('luminal_performance_mode') || 'high';
    const fpsCounter = localStorage.getItem('luminal_fps_counter') === 'true';
    const batterySaver = localStorage.getItem('luminal_battery_saver') === 'true';
    const cursorTheme = localStorage.getItem('luminal_cursor_theme') || 'default';
    const taskbarPos = localStorage.getItem('luminal_taskbar_position') || 'bottom';
    ...
    <div style="padding: 10px; height: 100%; box-sizing: border-box; overflow-y: auto; color: var(--text-color);">
        <div style="padding: 10px;">
            ...
            <h4><i class="fa-solid fa-gauge-high"></i> Performance &amp; Gaming</h4>
            ...
            <div class="settings-section">
                ...
                <p>Wallpaper:</p>
                <input type="text" class="settings-input" id="wallpaper-url-input" placeholder="Paste image URL...">
            ...
            <div class="settings-section">
                <p>Desktop Icon Size: <span id="icon-size-label">${iconSize}px</span></p>
                <input type="range" min="60" max="120" value="${iconSize}" oninput="updateIconSize(this.value)">
                <p>Taskbar Position:</p>
                <select class="settings-input" onchange="localStorage.setItem('luminal_taskbar_position', this.value); applyTaskbarSettings();">
                    <option value="bottom" ${taskbarPos === 'bottom' ? 'selected' : ''}>Bottom</option>
                    <option value="top" ${taskbarPos === 'top' ? 'selected' : ''}>Top</option>
                    <option value="left" ${taskbarPos === 'left' ? 'selected' : ''}>Left</option>
                    <option value="right" ${taskbarPos === 'right' ? 'selected' : ''}>Right</option>
                </select>
                <p>Taskbar Blur: <span id="blur-label">${savedBlur}px</span></p>
                <input type="range" min="0" max="40" value="${savedBlur}" oninput="localStorage.setItem('luminal_taskbar_blur', this.value); applyTaskbarSettings(); this.nextElementSibling.textContent=this.value+'px';"><span>${savedBlur}px</span>
                 <p>Taskbar Transparency:</p>
                <input type="range" min="0" max="1" step="0.05" value="${localStorage.getItem('luminal_taskbar_transparency') || '0.5'}" oninput="localStorage.setItem('luminal_taskbar_transparency', this.value); applyTaskbarSettings();">
            ...
            <div class="settings-section">
                 <p>Save or load your luminalOS settings and data.</p>
                 <button class="os-button" onclick="exportLumiSettings()">Export Data</button>
                 <button class="os-button" onclick="document.getElementById('import-lumi-input').click()">Import Data</button>
...
function renderDesktopIcons() { 
    ...
    let x = 20, y = 20; 
    const iconSize = parseInt(localStorage.getItem('luminal_iconSize') || 90);
    const xSpacing = iconSize + 10;
    const ySpacing = iconSize + 20;
...
function syncApps() {
    ...
    [...masterApps, ...savedApps].forEach(app => {
        if (!seen.has(app.name)) {
            ...
            if (masterApp && savedApp) {
                const finalApp = { ...masterApp, ...savedApp };
                finalApps.push(finalApp);
            } else if (masterApp) {
...
function showStartupPrompt() {
    if (localStorage.getItem('luminal_startup_complete')) return;
    const overlay = document.createElement('div');
    ...
    const showSecondPrompt = () => {
        overlay.innerHTML = `
            <div class="startup-modal">
                ...
                <p>Select how you want the tab to appear when automatically cloaked.</p>
    ...
    const showThirdPrompt = () => {
        overlay.innerHTML = `
            <div class="startup-modal">
                <h2>Choose Cloaking Mode</h2>
                <p>Pick how luminalOS should hide itself:</p>
                <div class="button-group">
                    <button class="os-button" data-mode="about">about:blank(reccomended option for all</button>
                    <button class="os-button" data-mode="blob">blob:(good for lightspeed and linewize filter users)</button>
                </div>
            </div>
        `;
        overlay.querySelectorAll('.os-button').forEach(button => {
            button.addEventListener('click', () => {
                localStorage.setItem('luminal_cloak_mode', button.dataset.mode);
                localStorage.setItem('luminal_auto_cloak', 'true');
                localStorage.setItem('luminal_startup_complete', 'true');
                overlay.remove();
                alert("Settings saved! Cloaking will apply next time you open luminalOS. Allow popups for best results.");
            });
        });
    };
    overlay.innerHTML = `
...
function init() {
    ...
    applyPerformanceMode(localStorage.getItem('luminal_performance_mode') || 'high');
    toggleFpsCounter(localStorage.getItem('luminal_fps_counter') === 'true');
    applyTaskbarSettings();
    applyCursorTheme(localStorage.getItem('luminal_cursor_theme') || 'default');
    checkBatterySaver();
